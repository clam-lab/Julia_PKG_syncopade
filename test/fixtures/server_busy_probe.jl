...
"""
    failing_probe(label::String, sleep_seconds::String) -> String

Hold one server job active for `sleep_seconds`, then throw an intentional error
containing the same concurrency evidence as `busy_probe`.
"""
function failing_probe(label::String, sleep_seconds::String)::String
    ...
    try
        ...
        error("intentional failure: $evidence")
    finally
        lock(active_lock) do
            active_jobs[] -= 1
            active_jobs[] >= 0 || error("active job counter became negative")
