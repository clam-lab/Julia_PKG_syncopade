...
@testset "Controlled conductor worker fixture" begin
    ...
    try
        ...
        @test fetch(accepted_task) == "controlled-job-1"
        ...
        @test getproperty.(request_entries, :kind) == [:status, :job, :job]
        @test getproperty.(response_entries, :kind) == [:status, :job, :job]
        @test getproperty.(response_entries, :value) == [
            "STATUS|idle",
            "ERROR|BUSY",
            "OK|STARTED|controlled-job-1",
        ]
        @test getproperty.(request_entries, :request_id) == [1, 2, 3]
        @test getproperty.(response_entries, :request_id) == [1, 2, 3]
    finally
